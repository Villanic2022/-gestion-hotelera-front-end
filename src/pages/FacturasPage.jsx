...
async function generarPDF(factura) {
    try {
        ...
        doc.setTextColor(...primaryColor);
        doc.setFont('helvetica', 'bold');
        doc.text('RESUMEN DE IMPORTES', 25, yPos + 8);
        ...
        if (facturaCompleta.cae && facturaCompleta.esFiscal) {
            ...
            doc.setFontSize(8);
            doc.setFont('helvetica', 'normal');
            doc.text('Verifique en www.afip.gob.ar/genericos/consultaCae', pageWidth - 25, yPos + 38, { align: 'right' });
            
        } else if (!facturaCompleta.esFiscal) {
            doc.setFillColor(254, 242, 242);
            doc.roundedRect(20, yPos - 5, pageWidth - 40, 25, 3, 3, 'F');
            doc.setDrawColor(248, 113, 113);
            doc.roundedRect(20, yPos - 5, pageWidth - 40, 25, 3, 3, 'S');
            
            doc.setFontSize(10);
            doc.setTextColor(185, 28, 28);
            doc.setFont('helvetica', 'bold');
            doc.text('COMPROBANTE NO VÁLIDO COMO FACTURA', 25, yPos + 8);
            doc.setFont('helvetica', 'normal');
            doc.text('Documento interno - No es comprobante fiscal', 25, yPos + 18);
        }
        ...
        const estadoColor = factura.estado === 'APROBADA' ? [16, 185, 129] : factura.estado === 'RECHAZADA' ? [239, 68, 68] : [245, 158, 11];
        doc.setTextColor(...estadoColor);
        doc.setFont('helvetica', 'bold');
        doc.text(factura.estado, 50, yPos);
    ...
    async function imprimirFactura(factura) {
        try {
            ...
            // Usar la misma lógica mejorada de generación de PDF
            ...
            // Resumen de importes con layout fiscal
            yPos += 80;
            doc.setFillColor(248, 250, 252);
            doc.roundedRect(20, yPos - 5, pageWidth - 40, 50, 3, 3, 'F');
            ...
            showToast('error', 'Error al imprimir factura');
        }
    }
                        ...
                        <div className="crud-modal" style={{ maxWidth: "600px" }}>
                            ...
                            <div style={{ padding: '20px' }}>
                                ...
                                <div style={{ display: 'grid', gap: '12px' }}>
                                    ...
                                    {facturaSeleccionada.emisorNombre && (
                                        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', paddingBottom: '10px' }}>
                                            <span style={{ color: '#6b7280', fontSize: '14px' }}>Emisor:</span>
                                            <span style={{ color: '#1f2937', fontWeight: '500' }}>{facturaSeleccionada.emisorNombre}</span>
                                        </div>
                                    )}
                                    
                                    {facturaSeleccionada.emisorDomicilioFiscal && (
                                        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', paddingBottom: '10px' }}>
                                            <span style={{ color: '#6b7280', fontSize: '14px' }}>Domicilio Fiscal:</span>
                                            <span style={{ color: '#1f2937', fontWeight: '500', fontSize: '13px' }}>{facturaSeleccionada.emisorDomicilioFiscal}</span>
                                        </div>
                                    )}
                                    ...
                                    {/* Información fiscal del receptor */}
                                    {facturaSeleccionada.receptorNombre && (
                                        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', paddingBottom: '10px' }}>
                                            <span style={{ color: '#6b7280', fontSize: '14px' }}>Cliente:</span>
                                            <span style={{ color: '#1f2937', fontWeight: '500' }}>{facturaSeleccionada.receptorNombre}</span>
                                        </div>
                                    )}
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', paddingBottom: '10px' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            {facturaSeleccionada.esFiscal !== undefined && (
                                                <span style={{
                                                    padding: '2px 8px',
                                                    borderRadius: '12px',
                                                    fontSize: '11px',
                                                    fontWeight: '600',
                                                    backgroundColor: facturaSeleccionada.esFiscal ? '#dcfce7' : '#fef2f2',
                                                    color: facturaSeleccionada.esFiscal ? '#166534' : '#dc2626'
                                                }}>
                                                    {facturaSeleccionada.esFiscal ? 'FISCAL' : 'NO FISCAL'}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* CAE destacado - solo si es fiscal */}
                                    {facturaSeleccionada.cae && facturaSeleccionada.esFiscal && (
                                        <div style={{
                                            ...
                                            <div style={{ fontSize: '12px', color: '#047857', textAlign: 'center', marginTop: '8px' }}>
                                                ✓ Comprobante autorizado por AFIP
                                            </div>
                                        </div>
                                    )}

                                    {/* Advertencia para comprobantes no fiscales */}
                                    {facturaSeleccionada.esFiscal === false && (
                                        <div style={{
                                            ...
                                            ⚠️ Comprobante no válido como factura fiscal
                                        </div>
                                    )}

                                    {/* CAE faltante en facturas fiscales */}
                                    {!facturaSeleccionada.cae && facturaSeleccionada.esFiscal === true && (
                                        <div style={{
                                            ...
                                            ⚠️ Esta factura fiscal no tiene CAE asignado
                                        </div>
                                    )}
